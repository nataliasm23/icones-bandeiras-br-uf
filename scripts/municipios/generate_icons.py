...
Usage:
  python3 scripts/municipios/generate_icons.py [--workers N] [--uf SP] [--skip-png] [--skip-svg]
"""
...
# Entry point
# ---------------------------------------------------------------------------
def run(*, dry_run=False, workers=4, uf=None, skip_png=False, skip_svg=False, limit=0):
    print(f"Loading municipality data from {MUNICIPIOS_JSON}...")
    municipios = load_municipios(filter_uf=uf)
    ...
    if dry_run:
        print("[DRY RUN] Would generate icons for the above municipalities.")
