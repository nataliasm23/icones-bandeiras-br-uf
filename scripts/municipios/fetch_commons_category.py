...
def run(*, dry_run=False):
    if dry_run:
        ...
        missing = sum(1 for m in municipios if m["flag_status"] != "found")
        print(f"  {missing} municipalities missing flags")
        print("  Source: Wikimedia Commons categories")
        return
    ...
    if matched:
        ...
        if errors:
            print(f"  {len(errors)} download errors")
