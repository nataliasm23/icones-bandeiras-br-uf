...
4. Detects coat of arms (brasao) files
...
NATIONAL_FLAG_INDICATORS = [
    "bandeira do brasil", "flag of brazil", "bandera de espana",
...
COAT_OF_ARMS_INDICATORS = [
    "brasao", "brasao", "coat of arms", "escudo", "seal", "selo",
]
...
def check_content(filepath: Path) -> dict:
    ...
    if ext == ".svg" or size < 1000:
        try:
            ...
            # Many Brazilian municipality flags legitimately contain brasao/escudo elements.
            # Only filename-based coat of arms detection (below) flags actual brasao files.
...
def run(*, dry_run=False):
    ...
    if dry_run:
        print(f"  [dry-run] Would deep-validate {len(all_files)} files. Exiting.")
        return
