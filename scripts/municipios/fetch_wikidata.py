...
def run(*, dry_run=False):
    ...
    if dry_run:
        print(f"\n  [DRY-RUN] fetch wikidata")
        print(f"  Municipalities missing flags: {len(missing)}")
        print(f"  Source: Wikidata SPARQL endpoint")
        print(f"  Would query: {WIKIDATA_SPARQL_URL}")
        return
    ...
    if not results:
        print("  No results from primary query. Trying simpler query...")
